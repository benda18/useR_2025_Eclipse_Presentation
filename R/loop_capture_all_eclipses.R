...
# calculate the next solar eclipse visible to input_locatoin
s.nextGlobEcl_Jd <- swe_sol_eclipse_when_glob(jd_start  = var_jultime, 
                                              ephe_flag = SE$FLG_MOSEPH, 
                                              ifltype   = 0, 
                                              backward  = F)$tret[c(1)]

l.nextGlobEcl_Jd <- swe_lun_eclipse_when_loc(jd_start  = var_jultime, 
                                             ephe_flag = SE$FLG_MOSEPH, 
                                             geopos    = c(input_lonlat["lon"], 
                                                           input_lonlat["lat"],0), 
                                             backward  = F)$tret[c(0,2,4)+1]

names(l.nextGlobEcl_Jd) <- c("time_of_max_eclipse", 
                             "time_of_partial_phase_start", 
                             "time_of_total_phase_start")

l.ecl_type <- ifelse(l.nextGlobEcl_Jd["time_of_total_phase_start"] == 0, 
                     yes = "partial", 
                     no  = "total")


# convert to gregorian time
s.nextEcl.temp <- swe_jdet_to_utc(jd_et    = s.nextGlobEcl_Jd, 
...
s.output_visible <- swe_sol_eclipse_how(jd_ut     = s.nextGlobEcl_Jd, 
                                        ephe_flag = SE$FLG_MOSEPH, 
                                        geopos    = c(input_lonlat["lon"], 
                                                      input_lonlat["lat"],0))
